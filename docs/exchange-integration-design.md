...
| account | 残高を持つ口座単位 | Main Wallet、Binance、Bybit |
| location | 資産の保管・運用場所 | Wallet、Aave、Binance |
| asset | 合算対象となる実体資産 | BTC、ETH、stETH、USDT |
| position | 資産と保管場所の組合せ | BinanceのETH 2.5 |
...
- 内部基準評価はUSD、画面ではUSDを主表示、JPYを補助表示として併記する。
- USD金額は小数2桁を常に表示し、JPY金額は小数を表示しない。
- JPY現金は `fiat:JPY` として保存する。
- ダッシュボードは `純資産`、`暗号資産小計`、`法定通貨小計`、`負債` を区別する。
- 法定通貨も総資産に含める。現行実装に「暗号資産のみ」切替はない。
...
- Bybit Unified Trading AccountのtotalEquityと通貨別評価を重ねない。
...
7. 個別更新では取得結果を同一リクエストのスナップショットとして保存し、一括更新では接続先単位の非同期ジョブとして保存する。

認証失敗や不完全レスポンスを0円として保存しない。失敗時は既存の最新スナップショットを残し、更新結果へエラー内容を表示する。

### 10.2 すべて更新

- 登録接続先を接続先単位で順番に取得する。
- API接続先とDeBankウォレットは、画面の個別更新・一括更新、またはmacOS launchdによる定期更新から取得する。
- 一括更新中は対象数、完了数、残り、成功数、失敗数、経過秒数を表示する。
...
- `launchd`から専用CLIを毎朝7:00、7:10、7:20、7:30、7:40、7:50に起動する。7:00は登録済みの有効ウォレットと認証済み取引所を全対象取得し、以降は前回失敗した対象だけを順番に再取得する。
- ダッシュボードのWebサーバーを起動しておく必要はない。認証情報は既存のmacOS Keychainから実行時だけ読み込む。
- Macが電源オフまたはログアウト中で実行できなかった場合は追いかけ実行しない。スリープ復帰後に7時台を過ぎて起動された場合も同様にスキップする。
- 定期更新と手動更新の重複を避けるため、定期更新CLIはロックファイルで同時実行を防ぐ。
- 再試行対象は日付単位の状態ファイルへ保存し、成功した対象を同じ7時台に再取得しない。7:50の再試行後も失敗した対象は次回実行日まで追いかけない。
- 成功したスナップショットだけを保存し、個別の失敗は既存の最新スナップショットを置き換えない。結果とエラーは `runs.jsonl` およびアプリログへ記録する。
...
| Method | Path | 用途 |
|---|---|---|
| GET | `/api/state` | 最新スナップショットを含む画面状態 |
| GET | `/api/history` | 保存済み履歴と実行履歴 |
| GET | `/api/providers` | 対応取引所一覧 |
| GET | `/api/sources` | 秘密を除いた接続先一覧 |
| POST | `/api/sources` | 接続先追加 |
| POST | `/api/sources/{source_id}` | 接続先削除。履歴は残す |
| POST | `/api/sources/{source_id}/credentials` | Keychainへ保存 |
| POST | `/api/sources/{source_id}/test` | 疎通テスト。残高は保存しない |
| POST | `/api/sources/{source_id}/preview` | 個別更新用の現在残高取得 |
| POST | `/api/sources/{source_id}/snapshots` | 個別更新結果の保存 |
| POST | `/api/sources/auto-import` | 取引所一括更新開始 |
| GET | `/api/sources/auto-import/{run_id}` | 取引所一括更新の進捗・結果 |
| POST | `/api/wallets` | ウォレット設定保存 |
| POST | `/api/wallets/auto-import` | 全ウォレット更新開始 |
| POST | `/api/wallets/auto-import/{wallet_id}` | 単一ウォレット更新開始 |
| GET | `/api/wallets/auto-import/{run_id}` | ウォレット更新の進捗・結果 |
| POST | `/api/import` | DeBank HTML解析結果の保存 |

個別取引所更新は取得結果を保存リクエストへ渡す。ウォレット一括更新と取引所一括更新はサーバー側ジョブが取得結果を保存する。
...
- プライマリーブルーは主要操作と選択状態に限定し、ダークモードでは本文リンクと主要操作で十分な明度差を持つ色を使い分ける。
...
- テーマはOS設定に追従しつつ、ヘッダーから明示的にライト／ダークを切り替え、選択を保存する。
- 操作の押下フィードバックは即時に返し、動きを減らす設定ではスライドや弾性運動を短いフェードへ置き換える。
...
ETH 合計 72.500000 ETH / $232,000
  ...
  Binance           15.000000    $48,000   20.7%
  Bybit              7.500000    $24,000   10.3%
```
...
| Binance | Exchange | $48,000 | 19.2% | 8分前 | 最新 |
| Bybit | Exchange | $22,420 | 9.0% | 8分前 | 一部未取得 |

各行を選択すると、同じ画面内に保管場所別の資産内訳を展開する。
...
| 接続先 | 方式 | 最終取得 | 状態 | 操作 |
|---|---|---|---|---|
| Main Wallet | DeBank自動取得 | 10分前 | 最新 | 更新 |
| Binance | API | 8分前 | 最新 | 更新 |
| Bybit | API | 8分前 | 一部未取得 | 更新 |
| bitFlyer | API | 2日前 | 認証エラー | 再試行 |

- 一括更新中は進行中、成功、要確認、失敗を行単位で表示する。
- 失敗しても成功済みデータを失わない。
- 完了時に成功・失敗件数、処理時間、失敗した接続先のエラー内容を出す。
- 終了後の主操作は「ダッシュボードへ戻る」。
...
異常や取得失敗を0円として保存しない。失敗した接続先は結果一覧で理由を示す。
...
- 初期対応: Binance、Bybit、bitFlyer、Coincheck、bitbank、GMOコイン
- 総資産: 暗号資産と取引所内法定通貨を含む純資産。小計と負債を併記
- 評価: USD内部基準。取得時点のUSD/JPYレートをスナップショットへ保存し、USD/JPYを併記
- 更新: 利用者操作、またはmacOS launchdによる毎朝7:00の定期実行
- 過去値: 保存済みスナップショットのみ
- 秘密情報: macOS Keychain
- 権限: 読取専用、出金権限禁止
